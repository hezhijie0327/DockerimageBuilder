...
function GetLatestVersion() {
    ...
    NEWAPI_VERSION=$(curl -s --connect-timeout 15 "https://api.github.com/repos/QuantumNous/new-api/git/matching-refs/tags" | jq -Sr ".[].ref" | grep "^refs/tags/v" | grep -v "alpha" | sort | tail -n 1 | sed "s/refs\/tags\/v//")
...
function GenerateReplacements() {
    replacement_list=(
        ...
        "s/{NEWAPI_VERSION}/${NEWAPI_VERSION_FIXED:-${NEWAPI_VERSION}}/g"
