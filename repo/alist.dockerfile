...
RUN \
    export WORKDIR=$(pwd) && mkdir -p "${WORKDIR}/BUILDKIT/alist-web" \
    && cd "${WORKDIR}/BUILDKIT/alist-web" \
...
RUN \
    ...
    && rm -rf "${WORKDIR}/BUILDTMP/ALIST/public/dist" \
    && mv "${WORKDIR}/BUILDTMP/alist-web/dist" "${WORKDIR}/BUILDTMP/ALIST/public/dist" \
    && ls -alh "${WORKDIR}/BUILDTMP/ALIST/public/dist" \
    && go build -o "${WORKDIR}/BUILDKIT/alist" -ldflags="-w -s -X github.com/alist-org/alist/v3/internal/conf.Version=${ALIST_CUSTOM_VERSION}" -tags=jsoniter .
