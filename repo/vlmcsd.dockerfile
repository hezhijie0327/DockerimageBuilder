...
RUN export WORKDIR=$(pwd) && mkdir -p "${WORKDIR}/BUILDKIT" "${WORKDIR}/BUILDTMP" && git clone -b master --depth=1 "https://github.com/Wind4/vlmcsd" "${WORKDIR}/BUILDTMP/VLMCSD" && cd "${WORKDIR}/BUILDTMP/VLMCSD" && make && cp -rf ${WORKDIR}/BUILDTMP/VLMCSD/bin/* "${WORKDIR}/BUILDKIT"
