...
RUN \
    export PACKAGE_MANAGER=$(sed -n '/"packageManager"/,/}/ s/.*"version": "\([^"]*\)".*/\1/p' package.json) \
...
COPY --from=build_rustfs_web /rustfs/out /rustfs/rustfs/static
...
ENV \
    RUSTFS_OBS_ENVIRONMENT="production" RUSTFS_OBS_LOGGER_LEVEL="info" \
    RUSTFS_CORS_ALLOWED_ORIGINS="*" RUSTFS_CONSOLE_CORS_ALLOWED_ORIGINS="*" \
    RUSTFS_ADDRESS=":9000" RUSTFS_SERVER_DOMAINS="localhost:9000" RUSTFS_COMPRESS_ENABLE="on" \
    RUSTFS_CONSOLE_ADDRESS=":9001" RUSTFS_CONSOLE_ENABLE="true" \    
    RUSTFS_ACCESS_KEY="rustfsadmin" RUSTFS_SECRET_KEY="rustfsadmin" \
    RUSTFS_VOLUMES="/data" \
    RUSTFS_TLS_PATH="" RUSTFS_TRUST_SYSTEM_CA="true"
