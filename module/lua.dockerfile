...
RUN export WORKDIR=$(pwd) && cat "/opt/package.json" | jq -Sr ".module.lua" > "${WORKDIR}/lua.json" && cat "${WORKDIR}/lua.json" | jq -Sr ".version" && cat "${WORKDIR}/lua.json" | jq -Sr ".source" > "${WORKDIR}/lua.autobuild"
