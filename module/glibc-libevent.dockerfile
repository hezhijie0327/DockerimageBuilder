...
RUN export WORKDIR=$(pwd) && cat "${WORKDIR}/package.json" | jq -Sr ".module.libevent" > "${WORKDIR}/libevent.json" && cat "${WORKDIR}/libevent.json" | jq -Sr ".version" && cat "${WORKDIR}/libevent.json" | jq -Sr ".source" > "${WORKDIR}/libevent.autobuild"
