...
RUN export LSBCodename=$( awk -F'=' '/^VERSION_CODENAME=/{ print $NF }' /etc/os-release ) \
    ...
    && apt update \
    && apt install -qy autoconf automake autopoint autotools-dev binutils bzip2 curl flex g++ gcc gifsicle git jq libbsd-dev libc6-dev libicu-dev libpng-dev libtool make nasm perl pkg-config python3 python3-pip unzip wget yacc zlib1g-dev gettext texinfo gawk bison python3-numpy graphviz re2c crossbuild-essential-amd64 crossbuild-essential-arm64 \
    && apt full-upgrade -qy \
    && apt autoremove -qy \
