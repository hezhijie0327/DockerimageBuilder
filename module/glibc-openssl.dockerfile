...
RUN export WORKDIR=$(pwd) && cat "${WORKDIR}/package.json" | jq -Sr ".module.openssl" > "${WORKDIR}/openssl.json" && cat "${WORKDIR}/openssl.json" | jq -Sr ".version" && cat "${WORKDIR}/openssl.json" | jq -Sr ".source" > "${WORKDIR}/openssl.autobuild"
