...
RUN export WORKDIR=$(pwd) && cat "/opt/package.json" | jq -Sr ".module.libmnl" > "${WORKDIR}/libmnl.json" && cat "${WORKDIR}/libmnl.json" | jq -Sr ".version" && cat "${WORKDIR}/libmnl.json" | jq -Sr ".source" > "${WORKDIR}/libmnl.autobuild"
