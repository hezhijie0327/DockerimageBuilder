...
RUN export WORKDIR=$(pwd) && cat "/opt/package.json" | jq -Sr ".module.libsodium" > "${WORKDIR}/libsodium.json" && cat "${WORKDIR}/libsodium.json" | jq -Sr ".version" && cat "${WORKDIR}/libsodium.json" | jq -Sr ".source" > "${WORKDIR}/libsodium.autobuild"
