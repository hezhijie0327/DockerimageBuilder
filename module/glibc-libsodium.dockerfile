...
RUN export WORKDIR=$(pwd) && curl -s --connect-timeout 15 "https://raw.githubusercontent.com/hezhijie0327/Patch/main/package.json" | jq -Sr ".module.libsodium" > "${WORKDIR}/libsodium.json" && cat "${WORKDIR}/libsodium.json" | jq -Sr ".version" && cat "${WORKDIR}/libsodium.json" | jq -Sr ".source" > "${WORKDIR}/libsodium.autobuild"
